...
| 13  | CardWeakness         | PokemonCardとTypeの多対多リレーション（弱点）。1枚のカードが複数の弱点を持つことを可能にする                                           |
| 14  | CardResistance       | PokemonCardとTypeの多対多リレーション（抵抗力）。1枚のカードが複数の抵抗力を持つことを可能にする                                       |
...
erDiagram
    ...
    PokemonCard ||--|{ CardWeakness : "links to"
    Type ||--|{ CardWeakness : "links to"

    PokemonCard ||--|{ CardResistance : "links to"
    Type ||--|{ CardResistance : "links to"
...
- **多対多の関係**:
  - カードの「タイプ」「特徴」「わざの属性」「レアリティ」「弱点」「抵抗力」は複数持つ可能性があるため、それぞれ中間テーブル (`CardType`, `CardSpecialFeature`, `CardMoveType`, `SpecialTrainer_Cards`, `CardWeakness`, `CardResistance`) を用いて表現します。
- **カテゴリによる属性の使い分け**:
