...
## Step 12: ポケモンカード以外の種別対応 (Data Model)

- **目的**: ポケモンカードだけでなく、トレーナーズカード（道具、サポート、スタジアムなど）も管理できるようにする。
- **やること**:
  - `PokemonCard` モデルに、カードの種別（ポケモン、道具、サポート、スタジアムなど）を識別するフィールドを追加する。
  - 種別に応じて、関係性のないフィールド（例: 道具カードには進化段階やタイプが不要）を適切にnull許可するか、種別ごとのモデルを検討する。
  - フォーム (`cards/forms.py`) やフィルタ (`cards/filters.py`) を修正し、種別による条件分岐や表示制御を行う。
  - テンプレート (`_card_item.html`, `_card_form.html`) を修正し、種別に応じた適切な表示を行う。
  - 管理画面 (`admin.py`) を更新し、新しいフィールドを編集可能にする。
- **ゴール**: ポケモンだけでなく、道具やサポートといったトレーナーズカードも同じアプリケーション内で一元管理できるようになること。

## Step 13: 接続URL固定 (Network)
...
## Step 15: お気に入りフラグ機能の実装 (Grouping)
