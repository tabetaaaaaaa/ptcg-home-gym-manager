...
- **やること**:
  ...
  - **ログイン機能の実装**(必須)
    ...
    - **AI精度改善へのオプトイン設定**: ユーザーが明示的に同意した場合のみ、アップロード画像をAIトレーニングに利用できるようにする（デフォルトOFF）。設定画面に切り替えUIを設ける。
  - **ライセンス・利用規約の整備**(必須)
    - READMEにAGPL-3.0ライセンスを明記する。
    - 利用規約を作成し、「ユーザーがアップロードしたコンテンツの著作権はユーザーに帰属」「AIトレーニングへの利用はオプトイン形式」「第三者への公開は行わない」旨を記載する。
    - プライバシーポリシーを作成し、データの取り扱いについて明記する。
  - **ヘルプ画面の実装**(必須):DONE
